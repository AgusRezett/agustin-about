		...
		<header className={style.Container}>
			...
			<div className={style.Menu}>
				<span className={style.Menu__item} onClick={() => languageContext.handleLanguageChange(isEnglishMode ? 'es' : 'en')}>
					<button className={style.Menu__item__button} aria-label={isEnglishMode ? 'English Mode' : 'Spanish Mode'}>
						<IoGlobe size={20} />
						{isEnglishMode ? <p>EN</p> : <p>ES</p>}
					</button>
				</span>
				<span className={style.Menu__item} onClick={() => toggleTheme(setIsDarkMode)}>
					<button className={style.Menu__item__button} aria-label={isDarkMode ? 'Toggle light mode' : 'Toggle dark Mode'}>
